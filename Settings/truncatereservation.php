                        ...
                        <div class="grandchild5">
                            <div class="grandchil6">
                                <?php
                                    ...
                                    try {
                                        $checkSql = "SELECT reservation_id FROM reservation WHERE reservation_id IN (SELECT reservation_id FROM payment) OR reservation_id IN (SELECT reservation_id FROM customer)";
                                        $checkResult = $conn->query($checkSql);

                                        if ($checkResult->num_rows > 0) {
                                            throw new Exception("Referenced");
                                        }

                                        $result = $conn->query("SELECT reservation_id FROM reservation");

                                        if ($result->num_rows > 0) {
                                            $conn->query("DELETE FROM reservation");
                                            $message = "<p style='font-size:20px; text-align:center;'>ALL RESERVATION RECORDS HAVE BEEN SUCCESSFULLY DELETED</p>";
                                        } else {
                                            $message = "<p style='font-size:20px; text-align:center;'>NO RESERVATIONS FOUND</p>";
                                        }

                                    } catch (Exception $e) {
                                        $message = "<p style='font-size:20px; text-align:center;'>CANNOT CLEAR RECORDS — RESERVATIONS ARE STILL BEING REFERENCED BY PAYMENT OR CUSTOMER RECORDS</p>";
                                    }
