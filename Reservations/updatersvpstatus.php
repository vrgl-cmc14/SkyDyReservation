                            ...
                            <div class="grandchil6">
                                <?php
                                    ...
                                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                                        ...
                                        $new_status = $_POST['statusrsvp'] ?? null;

                                        if ($reservation_id && $new_status) {
                                            $sql = "UPDATE reservation SET reservation_status = ? WHERE reservation_id = ?";
                                            $stmt = $conn->prepare($sql);
                                            $stmt->bind_param("si", $new_status, $reservation_id);

                                            if ($stmt->execute()) {
                                                $message = "<p style='font-size:20px; text-align:center;'>RESERVATION RECORD STATUS HAS BEEN UPDATED</p>";
                                            } else {
                                                $message = "<p style='font-size:20px; text-align:center;'>AN ERROR OCCURED</p>";
                                            }
                                        } else {
                                            $message = "<p>MISSING RESERVATION ID OR STATUS</p>";
                                        }
                                ...
                                <div class="gitna">
                                    <button class="backbackback" onclick="openToday()">BACK</button>
                                </div>
                                <br>
                            </div>
                        </div>
